...
void rxMidiLeds() {
  ...
  if (rxChannel == midiCh && rxType == 9) {           // if receiving NoteON on correct channel

    // cycle through FASTLEDs
    for (int i = 1; i < NUM_LEDS; i++) {              // cycle through all addressable LEDs - dont use LED 0!!

      if (rxType == 9 && rxPitch == currentProgram[i-1]) {              // if receiving noteON AND pitch matches LED
  ...
  if (rxChannel == midiCh && rxType == 8) {       // if receiving NoteOFF on correct channel

    // cycle through FASTLEDs
    for (int i = 1; i < NUM_LEDS; i++) {           // cycle through all addressable LEDs - dont use LED 0!!
      if (rxType == 8 && rxPitch == currentProgram[i-1]) {          // if receiving noteOFF AND pitch  matches LED
